...
class DataProcessing:
    ...
    def load_and_normalize_image(self, file_path):
        ...
        normalized_image = self.scaler.fit_transform(image.reshape(-1, image.shape[-1])).reshape(image.shape)
        return normalized_image

    def load_and_process_mask(self, file_path):
        """
    ...
    def crop_volume(self, volume, crop_size=(128, 128, 128), offsets=(56, 56, 13)):
        """
    ...
    def save_volume_and_mask(self, image, mask, img_idx, save_path):
        """
    ...
    def process_and_save_images(self, image_paths, mask_paths=None, save_path=None, crop_size=(128, 128, 128), offsets=(56, 56, 13)):
        """
        ...
        for img_idx in range(len(image_paths)):
            ...
            t2_image = self.load_and_normalize_image(image_paths[img_idx])
            t1ce_image = self.load_and_normalize_image(image_paths[img_idx])  # Adjust for available modalities
            flair_image = self.load_and_normalize_image(image_paths[img_idx])

            # Combine channels
            combined_image = np.stack([flair_image, t1ce_image, t2_image], axis=-1)

            # Crop image
            cropped_image = self.crop_volume(combined_image, crop_size, offsets)

            if mask_paths:
                # Load and process mask if masks are provided
                mask = self.load_and_process_mask(mask_paths[img_idx])
                cropped_mask = self.crop_volume(mask, crop_size, offsets)

                # Save image and mask
                self.save_volume_and_mask(cropped_image, cropped_mask, img_idx, save_path)
            else:
